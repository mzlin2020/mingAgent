import type { CallId, MessageId, TurnId } from '@xm/contracts';
 ...
 *
 * 崩溃恢复要能区分三种停法，因为每一种收尾时要补发的事件不一样（见 `crash-recovery.ts`
 ...
 *
 * 曾经还有第四种 `'permission'`（停在一个待批的权限请求上）。ADR-0039 之后判定不会挂起，
 * 于是那种停法在结构上不可能发生了——**它不是被简化掉的，是没有了产生它的路径。**
 *
 * 判定顺序对应真实的时间顺序（一条消息里可能带着待处理的工具调用，但工具调用不会
 * 反过来带着一条"正在流"的消息），所以这三种互斥，`if` 链就是完整的判定逻辑。
 *
 * ── `danglingToolUses`：一个批里排在后面、压根没轮到的调用 ──
 *
 * `runTurn()` 对一批并行 `tool_use` 是**顺序** `dispatchCall`（turn.ts）：一条消息可以带
 * 好几个 `tool_use` 块，若崩溃发生在处理第 2 个（正在执行）时，第 3 个及之后的调用
 * 连 `tool.start` 都没发过——它们在 `messages` 里的 `tool_use` 块
 * 没有任何配对的 `tool_result`，且不会出现在 `runningCalls` 里
 * （那个字段只认"已经被 dispatchCall 碰过"的调用）。这类调用如果不补一个 `tool.end`，
 * 续跑时喂给模型的上一条 assistant 消息里会有 `tool_use` 找不到匹配的 `tool_result`，
 * 这在 Anthropic 的 Messages API 里是硬错误，会让"继续"这条路直接失败。
 *
 * 只有 `'tool'` 这种停法可能有这类调用：`'message'` 时上一批（如果有）已经在
 * `dispatchCall` 的 `for` 循环里跑完才会轮到新一条消息开始流式输出，不会有遗留；
 * `'none'` 同理。
 */
 ...
 * 还是放弃），需要 `RunningCall.input`/`messageId` 这类
...
function danglingOf(state: SessionState): readonly DanglingToolUse[] {
  ...
  for (const b of assistant.blocks) {
    ...
    // 已经在 runningCalls 里的那一个是"卡住的那个"本身，由 kind === 'tool' 的
    // `calls` 字段承载，不算"没轮到"
