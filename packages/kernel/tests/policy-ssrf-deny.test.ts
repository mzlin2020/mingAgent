...
import { FALLBACK_ALLOW_RULE_ID, builtinRules, evaluate } from '@xm/kernel';
...
describe('🔴 M1-d DoD：web.fetch 解析到保留网段一律 deny', () => {
  ...
  it.each([
    ...
    const v = judge({ request: req('net.fetch', target), layers });
    expect(v.effect).toBe('deny');
    expect(v.ruleId).toBe('def.no-fetch-private-network');
  });

  it('公网地址不受影响 —— 防线不能宽到把一切都拦下', () => {
    const v = judge({ request: req('net.fetch', 'https://example.com/'), layers });
    expect(v.effect).toBe('allow');
    expect(v.ruleId).toBe(FALLBACK_ALLOW_RULE_ID);
  });

  it('兜底放行拦不过它 —— 它是一条普通 deny，排在兜底之前（ADR-0039）', () => {
    const v = judge({
