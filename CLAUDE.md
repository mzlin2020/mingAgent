...
决策见 ADR-0052 ~ 0061。**M3 的一切都还是文档，代码未开工**：插件容器、扩展点、
...
**M3 会在这张图上加一层、改两处**（规划，尚未实现，见 `docs/04 §1.1`）：
容器进 `kernel/src/container/`（纯逻辑零 I/O）；新增装配层 `@xm/compose`（profile 解析 +
patch 合并 + 装配期断言，包数 8 → 9，depcruise 加两条规则）；`turn.ts` 从焊死的流程变成
驱动器 + 命名扩展点。**依赖方向的硬约束一字不改**，且**不为插件新建细粒度包**——
容器化会让一部分依赖关系从 import 图挪到运行时，保住包边界规则是这个真损失的首要对策。
