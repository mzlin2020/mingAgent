...
| [M2-阶段划分](./M2-阶段划分.md) | M2-a 至 M2-i 的边界、依赖与独立验收条件 | 实施 M2 时 |
...
**M2-b“文本检索与结果展开”已完成，M2-c 尚未开始。** 当前安全基线仍为“主机范围自主 + 核心资源保护”：不设默认工作区沙箱，但数据、配置、密钥与安全实现对模型工具不可覆盖。M2-b 新增经路径网关执行的 `search.text`，以及只允许读取当前会话 `tool.end.fullRef` 的 `result.expand`，详见 [ADR-0042](./adr/0042-M2-b文本检索与会话内结果展开.md) 与 [M2 阶段划分](./M2-阶段划分.md)；M1.5 基线见 [ADR-0040](./adr/0040-M1.5自主安全边界与受控终端.md)。
