...
**M2-a“可见任务清单”已完成，M2-b 尚未开始。** 当前安全基线仍为“主机范围自主 + 核心资源保护”：不设默认工作区沙箱，但数据、配置、密钥与安全实现对模型工具不可覆盖。M2-a 新增 `todo.update`、持久回放与桌面进度视图，详见 [ADR-0041](./adr/0041-M2-a可见任务清单与窄事件写入口.md) 与 [M2 阶段划分](./M2-阶段划分.md)；M1.5 基线见 [ADR-0040](./adr/0040-M1.5自主安全边界与受控终端.md)。
