...
## 补记（2026-08-13，M2 复审）：子会话不进用户视野

第 3 节把"父子事件与上下文物理隔离"定成了存储事实——每次派生建一条带 `parentSessionId`
的真实会话。这是对的，但有两处消费方没有跟着区分内部会话与用户对话：

- 桌面端 `listSessions()` 不过滤，于是 Home 列表里会不断堆出「子 Agent：…」。
  那是内部执行细节，不是用户的对话。
- `scanForOrphanedSessions()` 也不过滤。崩溃后子会话既被第 4 节的
  `recoverInterruptedSubagents()` 从父会话那侧补 `subagent.end`，又被当成孤儿会话
  提示用户"要恢复吗"——同一件事两套恢复语义。而且启动时要全量回放每一条历史子会话，
  开销随派生次数线性增长。

两处都按 `parentSessionId !== undefined` 跳过。事件照旧完整落库，第 3 节"子轨迹仍可
单独诊断"不受影响。
