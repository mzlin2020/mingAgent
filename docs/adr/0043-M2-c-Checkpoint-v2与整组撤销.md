...
## 补记（2026-08-13，M2 复审）：内容总量没有预算

第 3 节写了"manifest 本身有条目数、路径长度和编码后的总字节上限"，实现也照做了——
但**内容 blob 的总字节一直没有上限**。后果不是理论的：模型一句 `rm -rf node_modules`
会让运行时把整棵树逐文件流进 BlobStore，几个 GB、几分钟，最后才在条目数上限处失败关闭。
而第 5 节的 GC 还没实现，已经写入的 blob 不回滚，那些字节永久占盘。

现在加了 256 MiB 的内容总量上限，用 `lstat` 的 size 在**流式写入之前**累计记账，
超预算立刻失败关闭并指出是哪个目标——所以超限时基本不会先写出一堆不可达 blob。
失败信息提示把目标缩小到更具体的路径。

第 5 节"GC 只能做可证明的图遍历"不变，仍未实现；这条补记只是把"失败也永久占盘"
这个已知缺口从"无界"收成"有界"。
